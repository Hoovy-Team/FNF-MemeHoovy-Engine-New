...
class LimeBackend implements IBackend
{
	...
	public function get(id:String):Bool
	{
		var symbol = new IdAndLibrary(id, modLibraries);
		var j = symbol.library.get(symbol.modId);
		return j;
	}
...
class LimeModLibrary extends AssetLibrary
{
	...
	/**
	 * Works almost the same as exists, but with some differences.
	 */
	public function get(id:String):Bool
	{
		var symbol = new IdAndLibrary(id);
		...
		else
		{
			...
			return openfl.utils.Assets.exists(id);
		}
		// No fallback.
		return false;
	}
